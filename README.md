<p align="center">
  <img src="./public/brand/care-for-all-logo.png" alt="케어포올 로고" width="180" />
</p>

<h1 align="center">케어포올</h1>

<p align="center">
  요양보호사의 짧은 관찰 메모를 정돈된 요양보호기록 초안으로 바꿔주는 웹 애플리케이션입니다.
</p>

<p align="center">
  대상자와 날짜를 선택해 그날의 일을 입력하면, 케어포올이 일일 기록과 주간 기록 초안을 만들어 줍니다.
</p>

## 기획 의도
요양보호사 일을 하는 어머니 박정원 여사가 퇴근 후에도 요양 일지를 작성하며 쉬지 못하는 모습이 안타까워서 제작을 하게 되었습니다. 

요양보호일지 작성에서 체크, 식사 횟수, 대소변 횟수 등은 매일 비슷하여 큰 고민 없이 빠르게 입력이 가능합니다. 그러나 당일의 일을 하나의 정돈된 문장으로 만드는 란에서 좋은 문장을 만드는데 고민을 하는 과정에서 병목이 발견되었습니다.

케어포올은 사용자가 사건을 입력하면 AI가 대신 문장을 만듭니다. 사용자에게 완벽한 문장을 떠올리는게 아닌 단순 사실만을 입력하도록 유도하여 사용자가 일지를 작성하면서 주로 겪는 병목을 제거하여 빠른 일지 작성을 돕습니다.

## 주요 기능

- 대상자 등록과 날짜별 기록 관리
- 관찰 메모를 일일 인지·행동 기록으로 변환
- 생성 결과 검토, 수정, 복사, 삭제
- 일일 기록을 바탕으로 주간 기록 생성
- 교체 가능한 AI 공급자와 SQLite를 이용한 단일 서버 운영

## 로컬 실행

```bash
pnpm install
cp .env.example .env
mkdir -p app_data
pnpm db:push
pnpm dev
```

기본 설정은 로컬 Ollama를 사용합니다. OpenAI 호환 API로 전환하는 방법은
[운영 가이드](./docs/operations.md#ai-공급자-설정)를 참고하세요. 개발 서버는
기본적으로 <http://localhost:3000>에서 실행됩니다.

## 문서

제품 원칙, 시스템 구조, 배포 절차와 향후 작업은
[문서 안내](./docs/README.md)에서 찾을 수 있습니다.

## 저장소 구조

```text
src/app/         화면, Route Handler, Server Action
src/components/  여러 화면에서 공유하는 UI
src/features/    기능 단위 쿼리, 타입, 키
src/db/          Drizzle 연결과 스키마
src/lib/         AI와 날짜 등 외부·공통 로직
scripts/         배포와 백업 스크립트
docs/            제품, 아키텍처, 운영, 로드맵
```

패키지 명령은 [`package.json`](./package.json), 환경변수 예시는
[`.env.example`](./.env.example), 데이터 모델은
[`src/db/schema.ts`](./src/db/schema.ts)가 기준입니다.
