...
export function AddRecipientForm() {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger className="text-base font-bold tracking-widest text-black hover:text-surface-500 transition-colors">
        + 새 어르신 추가
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-10 w-[90vw] max-w-md shadow-2xl z-50 outline-none border border-surface-200 rounded-2xl">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <div className="flex justify-end gap-6 pt-4">
              <Dialog.Close className="text-base font-bold tracking-widest text-surface-500 hover:text-black transition-colors">
                취소
              </Dialog.Close>
              <button
                type="submit"
